...
/**
 * 認証必須だが、userIdをアクションに渡さないServer Action用高階関数
 *
 * 使用例:
 * // アクションはuserIdを受け取らない
 * const myAction = ensureAuthenticated(async (data) => {
 * // 認証済みユーザーのみが実行できる処理
 * });
 */
export function ensureAuth<T extends unknown[], R>(
  // この action は userId を引数に取らない
  ...
  return async (...args: T): Promise<R> => {
    ...
    // チェックをパスしたら、userId を渡さずにアクションを呼び出す
    return action(...args);
  };
}
