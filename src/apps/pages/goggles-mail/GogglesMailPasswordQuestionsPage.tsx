...
export const GogglesMailPasswordQuestionsPage: React.FC<MailPageProps> = ({ onPhaseNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await validateSecretQuestion({
      ...
      if (result.success) {
        ...
        setError(result.error || '回答の確認に失敗しました');
      }
    } catch {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center bg-white px-4">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">秘密の質問</p>
      </div>

      <div className="w-full max-w-md bg-white border border-gray-200 rounded-lg shadow-sm p-8">
        <div className="mb-6">
          <div className="text-sm text-gray-500 mb-2">質問 {currentQuestion}/3</div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {isSubmitting ? '確認中...' : (currentQuestion < 3 ? '次へ' : '完了')}
          </button>
        </form>
